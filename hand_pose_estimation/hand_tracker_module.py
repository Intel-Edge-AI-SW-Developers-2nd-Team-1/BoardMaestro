...
class HandTracker:
    def __init__(self, input_src=None,
                pd_xml="ai_modeling/model/palm_detection_FP32.xml", 
                pd_device="CPU",
                pd_score_thresh=0.5, pd_nms_thresh=0.3,
                use_lm=True,
                lm_xml="ai_modeling/model/hand_landmark_FP32.xml",
                lm_device="CPU",
    ...
    def load_models(self, pd_xml, pd_device, lm_xml, lm_device):
        ...
        pd_bin = pd_xml.replace(".xml", ".bin")
        print("Palm Detection model - Reading network files:\n\t{}\n\t{}".format(pd_xml, pd_bin))
        self.pd_net = self.core.read_model(model=pd_xml)
        ...
        self.pd_input_blob = self.pd_net.input().get_any_name()
        print(f"Input blob: {self.pd_input_blob} - shape: {self.pd_net.input().partial_shape}")
        self.pd_w, self.pd_h = self.pd_net.input().shape[2:4]
        for o in self.pd_net.outputs:
            print(f"Output blob: {o.get_any_name()} - shape: {o.shape}")
            if "classificators" in o.get_any_name():
                self.pd_scores = o.get_any_name()
            if "regressors" in o.get_any_name():
                self.pd_bboxes = o.get_any_name()
        print("Loading palm detection model into the plugin")
        self.pd_exec_net = self.core.compile_model(model=self.pd_net, device_name=pd_device)
        ...
        if self.use_lm:
            ...
            lm_bin = lm_xml.replace(".xml", ".bin")
            print("Landmark model - Reading network files:\n\t{}\n\t{}".format(lm_xml, lm_bin))
            self.lm_net = self.core.read_model(model=lm_xml)
            ...
            self.lm_input_blob = self.lm_net.input().get_any_name()
            print(f"Input blob: {self.lm_input_blob} - shape: {self.lm_net.input().partial_shape}")
            self.lm_w, self.lm_h = self.lm_net.input().shape[2:4]
            for o in self.lm_net.outputs:
                print(f"Output blob: {o.get_any_name()} - shape: {o.shape}")
            ...
            self.lm_exec_net = self.core.compile_model(model=self.lm_net, device_name=lm_device)
    ...
    def run(self):
        ...
        while True:
            ...
            # Get palm detection
            pd_rtrip_time = now()
            inference = self.pd_exec_net([frame_nn])
            ...
            if self.use_lm:
                for i,r in enumerate(self.regions):
                    ...
                    # Get hand landmarks
                    lm_rtrip_time = now()
                    inference = self.lm_exec_net([frame_nn])
